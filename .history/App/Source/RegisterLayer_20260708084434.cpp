...
void RegisterLayer::OnRender()
{
	...
	if (ImGui::Begin("Register", nullptr, flags))
	{
		...
		if (strcmp(m_Password, m_ConfirmPassword) != 0)
		{
			m_NotifationMessage = "Passwords do not match!";
			ImGui::TextColored(ImVec4(1.0f, 0.3f, 0.3f, 1.0f), "%s", m_NotifationMessage.c_str()); 
			ImGui::Spacing();
		}
		else
		{
			if (ImGui::Button("Register", ImVec2(inputWidth, 0)))
			{
				...
				{
					m_NotifationMessage = "Username cannot be empty.";
				}
				else if (strlen(m_Password) == 0)
				{
					m_NotifationMessage = "Password cannot be empty.";
					std::memset(m_Password, 0, sizeof(m_Password));
				}
				else if (strlen(m_ConfirmPassword) == 0)
				{
					m_NotifationMessage = "Please confirm your password.";
